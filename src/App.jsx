      ...
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            {position && <button onClick={() => { if (window.confirm("Delete this position?")) onDelete(position.id); }} style={{ background: "rgba(239,68,68,.15)", color: "#ef4444", border: "1px solid rgba(239,68,68,.3)", borderRadius: 10, padding: "11px 18px", cursor: "pointer", fontFamily: "inherit", fontWeight: 700, fontSize: 13, whiteSpace: "nowrap" }}>Delete</button>}
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  const [trades, setTrades] = useState(() => { const d = loadData(); return d?.trades || SEED_TRADES; });
  const [shares, setShares] = useState(() => { const d = loadData(); return d?.shares || SEED_SHARES; });
